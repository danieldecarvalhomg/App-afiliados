-- Permite repetir pela nuvem uma conversão que caiu no fallback da extensão.
...
BEGIN
  ...
  WHERE id=p_conversion_id AND user_id=p_user_id
    AND status IN (
      'resolution_failed','conversion_failed','affiliate_account_not_configured',
      'invalid_url','awaiting_companion'
    );
  v_reset := FOUND;
  IF v_reset THEN
