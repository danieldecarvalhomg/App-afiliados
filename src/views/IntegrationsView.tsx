...
export const IntegrationsView: React.FC = () => {
  ...
  const marketplaceCards: Array<{ platform: ConfigurableAffiliatePlatform; name: string; description: string }> = [
    ...
    { platform: "mercado_livre", name: "Mercado Livre", description: "Motor próprio de conversão no backend, disponível no computador e no celular." },
  ];
  ...
  const mercadoLivreConversionMode = mercadoLivreDirectConfigured
    ? "Conversão somente pelo backend"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {marketplaceCards.map((integration) => {
              const configured = integration.platform === "mercado_livre"
                ? Boolean(mercadoLivreAccount?.configured)
        ...
        >
          ...
          >
            ...
            {marketplaceConfigOpen === "mercado_livre" && <>
              ...
              <p className="mt-2 text-[11px] leading-5 text-[#64748B]">Essas credenciais são usadas para consultar itens e preços no Radar. Depois da conexão inicial da sessão, a geração do link afiliado acontece exclusivamente no backend e não exige Chrome aberto.</p>
            </>}
            <button
              disabled={busy !== null}
        ...
        <div className="fixed inset-0 z-[100] flex items-stretch justify-center bg-white p-0 sm:items-center sm:bg-black/40 sm:p-4" onMouseDown={() => setMercadoLivreOpen(false)}>
          <div className="h-[100dvh] w-full overflow-y-auto overscroll-contain bg-[#FFFFFF] px-4 pb-[calc(1rem+env(safe-area-inset-bottom))] pt-0 sm:h-auto sm:max-h-[92vh] sm:max-w-2xl sm:rounded-2xl sm:border sm:border-[#E8E9ED] sm:p-6" onMouseDown={(event) => event.stopPropagation()}>
            <div className="sticky top-0 z-30 -mx-4 flex items-start justify-between gap-4 border-b border-[#E8E9ED] bg-white/95 px-4 pb-3 pt-[calc(0.75rem+env(safe-area-inset-top))] backdrop-blur sm:static sm:mx-0 sm:border-0 sm:bg-transparent sm:p-0">
              <div className="min-w-0"><h2 className="text-base font-medium leading-6 text-[#0F172A] sm:text-lg">Mercado Livre — Programa de Afiliados</h2><p className="mt-1 hidden text-xs text-[#9CA3AF] sm:block">O conector sincroniza a sessão uma vez; depois o motor próprio converte no backend, inclusive para pedidos feitos no celular.</p></div>
            ...
            {!mercadoLivreDirectConfigured && <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">{mercadoLivreCompanionOnline ? "Abra o Mercado Livre no mesmo Chrome conectado ao Companion e faça login uma única vez. Depois de alguns segundos, clique em Atualizar status para confirmar a sincronização. As próximas conversões serão feitas no backend, mesmo com o Chrome fechado." : "Instale ou atualize o Conector AfiliHub, pareie-o e faça a conexão inicial do Mercado Livre. A sessão será sincronizada e ficará cifrada no backend; o Chrome não será necessário para as conversões seguintes."}</div>}
            ...
            {mercadoLivreDirectConfigured && <div className="mt-6 rounded-xl border border-[#E8E9ED] bg-[#F8FAFC] p-4"><p className="text-sm font-medium text-[#0F172A]">Testar geração no backend</p><p className="mt-1 text-xs text-[#475569]">Informe uma URL direta de produto MLB ou universal MLBU. O teste usa exclusivamente a sessão cifrada no servidor.</p><div className="mt-3 flex flex-col gap-2 sm:flex-row"><input value={mercadoLivreTestUrl} onChange={(event)=>setMercadoLivreTestUrl(event.target.value)} placeholder="https://www.mercadolivre.com.br/.../MLB... ou /up/MLBU..." className="min-w-0 flex-1 rounded-lg border border-[#CBD5E1] bg-[#FFFFFF] px-3 py-2 text-sm text-[#0F172A] placeholder:text-[#64748B]"/><button onClick={testMercadoLivreGeneration} disabled={busy !== null || !mercadoLivreTestUrl.trim()} className="rounded-lg bg-[#E2E8F0] px-4 py-2 text-sm font-medium text-[#0F172A] disabled:opacity-50">{busy === "ml:test" ? "Gerando…" : "Testar geração"}</button></div>{mercadoLivreTestJob && <div className={`mt-3 rounded-lg border p-3 text-xs ${mercadoLivreTestJob.status === "SUCCESS" ? "border-green-200 bg-green-50 text-green-800" : ["FAILED","EXPIRED"].includes(mercadoLivreTestJob.status) ? "border-red-200 bg-red-50 text-red-800" : "border-blue-200 bg-blue-50 text-blue-800"}`}><p>Status: {mercadoLivreTestJob.status}{mercadoLivreTestJob.errorCode ? ` · ${mercadoLivreTestJob.errorCode}` : ""}</p>{mercadoLivreTestJob.resultUrl && <a href={mercadoLivreTestJob.resultUrl} target="_blank" rel="noreferrer" className="mt-1 block break-all underline">{mercadoLivreTestJob.resultUrl}</a>}</div>}</div>}
