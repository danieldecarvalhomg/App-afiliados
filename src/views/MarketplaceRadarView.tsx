...
interface Props { onConfigureShopee: () => void; onCreateManual: () => void; onPrepared?: (product: ProductRecord) => void; }
...
export const MarketplaceRadarView: React.FC<Props> = ({ onConfigureShopee, onCreateManual, onPrepared }) => {
  ...
  const marketplacePicker = <select value={marketplace} onChange={(event) => chooseMarketplace(event.target.value)} className="rounded border border-[#E8E9ED] bg-[#F8FAFC] p-2 text-white"><option value="shopee">Shopee</option><option value="amazon">Amazon</option><option value="mercado_livre">Mercado Livre</option></select>;
  ...
  if (error && affiliateStatus !== 'valid') return <div className="space-y-6"><h1 className="text-2xl font-bold">Radar de Ofertas</h1><div>{marketplacePicker}</div><div className="rounded-xl border border-red-500/30 bg-red-500/10 p-6 text-red-200">{error}</div></div>;
  if (affiliateStatus !== 'valid') {
    const pending = affiliateStatus === 'pending_validation';
    return <div className="space-y-6"><div><h1 className="text-2xl font-bold">Radar de Ofertas</h1><div className="mt-4">{marketplacePicker}</div></div><div className="rounded-xl border border-[#E8E9ED] bg-[#FFFFFF] px-6 py-16 text-center"><LockKeyhole className="mx-auto mb-4 h-9 w-9 text-[#9CA3AF]" /><h2 className="text-lg font-semibold">{pending ? `Validando sua conta ${marketplaceName}...` : `Conecte e valide sua conta ${marketplaceName}`}</h2><p className="mx-auto mt-2 max-w-lg text-sm text-[#6B6F7B]">{pending ? 'O Radar será liberado após a validação das credenciais.' : 'Abra Integrações, informe as credenciais oficiais e volte ao Radar.'}</p>{!pending && <button onClick={onConfigureShopee} className="mt-6 rounded-lg bg-[#EDEDED] px-4 py-2 text-sm font-medium text-[#111]">Abrir integrações</button>}</div></div>;
  }
  ...
  return <div className="space-y-6">
    ...
    {marketplace === 'mercado_livre' && <div className="rounded-lg border border-violet-500/30 bg-violet-500/10 p-3 text-sm text-violet-200">O catálogo usa a API autenticada do Mercado Livre e é verificado automaticamente. Mudanças são entregues ao app pelo Supabase Realtime; ao preparar uma oferta, o Companion gera o link com comissão em segundo plano.</div>}
    {notice && <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/10 p-3 text-sm text-emerald-600">{notice}</div>}{error && <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-3 text-sm text-amber-200">{error}</div>}
    <div className="flex flex-wrap gap-3 rounded-xl border border-[#E8E9ED] bg-[#FFFFFF] p-4"><label className="text-sm text-[#6B6F7B]">Marketplace <span className="ml-2">{marketplacePicker}</span></label><label className="text-sm text-[#6B6F7B]">Categoria<input value={filters.category ?? ''} onChange={e => setFilters({ ...filters, category: e.target.value || undefined })} className="ml-2 w-28 rounded border border-[#E8E9ED] bg-[#F8FAFC] p-2 text-white" /></label><label className="text-sm text-[#6B6F7B]">Preço mín.<input type="number" value={filters.minPrice ?? ''} onChange={e => setFilters({ ...filters, minPrice: e.target.value ? Number(e.target.value) : undefined })} className="ml-2 w-20 rounded border border-[#E8E9ED] bg-[#F8FAFC] p-2 text-white" /></label><label className="text-sm text-[#6B6F7B]">Preço máx.<input type="number" value={filters.maxPrice ?? ''} onChange={e => setFilters({ ...filters, maxPrice: e.target.value ? Number(e.target.value) : undefined })} className="ml-2 w-20 rounded border border-[#E8E9ED] bg-[#F8FAFC] p-2 text-white" /></label><label className="text-sm text-[#6B6F7B]">Desconto mín.<input type="number" value={filters.minDiscount ?? ''} onChange={e => setFilters({ ...filters, minDiscount: e.target.value ? Number(e.target.value) : undefined })} className="ml-2 w-20 rounded border border-[#E8E9ED] bg-[#F8FAFC] p-2 text-white" /></label><label className="text-sm text-[#6B6F7B]">Comissão mín.<input type="number" value={filters.minCommission ?? ''} onChange={e => setFilters({ ...filters, minCommission: e.target.value ? Number(e.target.value) : undefined })} className="ml-2 w-20 rounded border border-[#E8E9ED] bg-[#F8FAFC] p-2 text-white" /></label><label className="flex items-center gap-2 text-sm text-[#6B6F7B]"><input type="checkbox" checked={filters.freeShipping ?? false} onChange={e => setFilters({ ...filters, freeShipping: e.target.checked })} />Frete grátis</label><label className="flex items-center gap-2 text-sm text-[#6B6F7B]"><input type="checkbox" checked={filters.coupon ?? false} onChange={e => setFilters({ ...filters, coupon: e.target.checked })} />Cupom</label><label className="text-sm text-[#6B6F7B]">Ordenar<select value={filters.sort} onChange={e => setFilters({ ...filters, sort: e.target.value as RadarSort })} className="ml-2 rounded border border-[#E8E9ED] bg-[#F8FAFC] p-2 text-white"><option value="score">Melhor oportunidade</option><option value="discount">Maior desconto</option><option value="commission">Maior comissão</option><option value="sales">Mais vendidos</option><option value="price">Menor preço</option><option value="recent">Mais recentes</option></select></label></div>
