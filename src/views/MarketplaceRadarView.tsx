...
export const MarketplaceRadarView: React.FC<Props> = ({ onConfigureMarketplace, onCreateManual, onPrepared }) => {
  ...
  return <div className="space-y-6">
    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><div className="flex flex-wrap items-center gap-2"><h1 className="text-2xl font-bold">Radar de Ofertas</h1><span className={`rounded-full border px-2 py-0.5 text-[11px] font-semibold ${liveConnected ? 'border-emerald-600 bg-emerald-50 text-emerald-800' : 'border-amber-600 bg-amber-50 text-amber-900'}`}>{liveConnected ? 'Ao vivo' : 'Sincronizando'}</span></div><p className="mt-1 text-sm text-[#6B6F7B]">Ofertas da sua conta {marketplaceName} validada.</p><p className="mt-1 text-xs text-slate-600">Atualização automática diária às 12h (horário de Brasília). Última consulta: {dateTime(latestSeen)}</p></div><button onClick={() => void refresh()} disabled={refreshing} className="flex items-center gap-2 rounded-lg bg-blue-700 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-800 disabled:opacity-50"><RefreshCw className={`h-4 w-4 ${refreshing ? 'animate-spin' : ''}`} />Atualizar</button></div>
    ...
    {deals.length === 0 ? <div className="rounded-xl border border-[#E8E9ED] bg-[#FFFFFF] py-16 text-center"><ShoppingBag className="mx-auto mb-3 text-slate-600" /><h2 className="font-semibold">Nenhuma oferta encontrada nesta conta</h2><p className="mt-1 text-sm text-[#6B6F7B]">Você também pode <button onClick={onCreateManual} className="font-medium text-blue-700 underline">cadastrar um produto manualmente</button>.</p></div> : <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{deals.map(deal => <article key={deal.id} className="flex flex-col rounded-xl border border-[#E8E9ED] bg-[#FFFFFF] p-5"><div className="flex h-40 items-center justify-center overflow-hidden rounded-lg bg-[#F8FAFC]">{deal.imageUrl ? <img src={deal.imageUrl} alt={deal.title} className="h-full w-full object-cover" /> : <ShoppingBag className="text-slate-600" />}</div><div className="mt-4 flex items-start justify-between gap-3"><h2 className="font-semibold leading-snug">{deal.title}</h2><span className="shrink-0 rounded bg-violet-100 px-2 py-1 text-xs font-semibold text-violet-800">Score {deal.dealScore ?? 0}</span></div><p className="mt-2 text-lg font-bold">{money(deal.price)}</p>{deal.originalPrice != null && <p className="text-sm text-slate-600 line-through">{money(deal.originalPrice)}</p>}<div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-700">{deal.discountPercent != null && <span>{deal.discountPercent}% off</span>}{deal.commissionRate != null && <span>Comissão {deal.commissionRate}%</span>}{deal.salesCount != null && <span>{deal.salesCount} vendas</span>}{deal.rating != null && <span>★ {deal.rating}</span>}{deal.coupon && <span>Com cupom</span>}{deal.freeShipping && <span>Frete grátis</span>}</div>{deal.scoreReasons?.length ? <p className="mt-3 text-xs text-slate-600">Em alta: {deal.scoreReasons.join(' · ')}</p> : null}<div className="mt-auto flex gap-2 pt-5"><a href={deal.productUrl} target="_blank" rel="noreferrer" className="flex flex-1 items-center justify-center gap-1 rounded-lg border border-slate-400 py-2 text-sm font-medium text-slate-800 hover:bg-slate-50"><ExternalLink className="h-4 w-4" />Ver oferta</a><button onClick={() => deal.id && void prepare(deal.id)} disabled={preparing === deal.id} className="flex flex-1 items-center justify-center gap-1 rounded-lg bg-blue-700 py-2 text-sm font-semibold text-white hover:bg-blue-800 disabled:opacity-50">{preparing === deal.id ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />}{marketplace === 'mercado_livre' ? 'Preparar produto' : 'Preparar promoção'}</button></div></article>)}</div>}
  </div>;
};
