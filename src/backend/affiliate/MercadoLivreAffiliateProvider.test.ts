...
describe('MercadoLivreAffiliateProvider', () => {
  it('delega a geração ao adapter com contexto do usuário e tracking', async () => {
    const generate = vi.fn(async () => ({ affiliateUrl: 'https://meli.la/abc123', itemId: 'MLB1234567', trackingLabel: 'promofy_manual', cached: false }));
    ...
    expect(generate).toHaveBeenCalledWith('user-a', 'https://produto.mercadolivre.com.br/MLB-1234567', 'promofy_manual', null);
    expect(result).toMatchObject({ success: true, convertedUrl: 'https://meli.la/abc123', provider: 'mercado_livre_remote_browser_v1' });
  });
