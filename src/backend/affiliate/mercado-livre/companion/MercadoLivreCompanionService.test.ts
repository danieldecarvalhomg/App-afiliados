...
const instance = {
  id: 'instance-a', userId: 'user-a', name: 'Chrome', status: 'ONLINE', extensionVersion: '1.2.1',
...
describe('MercadoLivreCompanionService', () => {
  ...
  it('emite credencial própria limitada e nunca a persiste em texto puro', async () => {
    const repo = repository();
  const result = await new MercadoLivreCompanionService(repo as any, {} as any).pair({ code: 'ABCD-EFGH-JKLM', name: 'Chrome', extensionVersion: '1.2.1', adapterVersion: 5 });
  ...
  it('retoma jobs que exigiam ação quando a sessão volta a READY', async () => {
    const repo = repository();
  await new MercadoLivreCompanionService(repo as any, {} as any).heartbeat(instance as any, { mercadoLivreStatus: 'READY', extensionVersion: '1.2.1', adapterVersion: 5 });
  ...
  expect(result.required).toMatchObject({ extensionVersion: '1.2.1', adapterVersion: 5 });
  });
